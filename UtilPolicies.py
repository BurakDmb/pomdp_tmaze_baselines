...
class LSTMACPolicy(ActorCriticPolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = LSTMACNetwork(self.features_dim, self.net_arch)


class LSTMACNetwork(nn.Module):
    """
    Custom network for policy and value function.
    It receives as input the features extracted by the feature extractor.
    ...
    def __init__(
        ...
        super(LSTMACNetwork, self).__init__()
        ...
        # it could be parameterized in the future.
        num_layers = 1
        self.LSTM = nn.LSTM(feature_dim, feature_dim,
                            num_layers=num_layers, batch_first=True)
        self.LSTMInput = feature_dim
    ...
    def forward(self, features: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        :return: (th.Tensor, th.Tensor) latent_policy, latent_value of the
            ...
            If all layers are shared, then ``latent_policy == latent_value``
        """
        inputLstm = features.unsqueeze(1)
        output, (hx, cx) = self.LSTM(inputLstm)
        hn = hx.view(-1, self.LSTMInput)

        shared = self.shared_net(hn)
        return self.policy_net(shared), self.value_net(shared)
